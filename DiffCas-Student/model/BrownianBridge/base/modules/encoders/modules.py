...
class CT_transformer(nn.Module):
    ...
    def forward(self, x_cond_latent, attmap, neighbor_dest):
        # return x_cond_latent, 0
     ...
     #   y = attmap
        ...
        x_attn, attn_weights,  loss_to_learn_from_attmap = self.cross_attn(x, y)

        x_attn = x_attn.permute(0, 2, 1)
        x_attn = x_attn.view(x_attn.shape[0], x_attn.shape[1], 16, 16)
        residual = self.nc_up(x_attn)
        x_attn = residual + x_cond_latent
