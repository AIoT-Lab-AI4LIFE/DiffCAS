...
class BaseRunner(ABC):
    ...
    def initialize_model_optimizer_scheduler(self, config, is_test=False):
        ...
        if not is_test:
            ...
            img_size = 256
            n_channels = 1
            self.R_A = Reg(img_size, img_size, n_channels, n_channels).to(config.training.device[0]) #//
            self.spatial_transform = Transformer_2D().to(config.training.device[0]) #// 
    ...
    def load_model_from_checkpoint(self):
        ...
        if self.config.model.__contains__('model_load_path') and self.config.model.model_load_path is not None:
            ...
            # if 'reg' in model_states.keys(): 
            #     self.R_A.load_state_dict(model_states['reg'])
    ...
    def get_checkpoint_states(self, stage='epoch_end'):
        ...
        if self.config.training.use_DDP:
            ...
            model_states['reg'] = self.R_A.state_dict()
    ...
    def validation_epoch(self, val_loader, epoch):
        ...
        for val_batch in pbar:
            loss_recon, loss_cos_sim = self.loss_fn(net=self.net,
                                batch=val_batch,
                            r_a = self.R_A, 
                            spatial_transform=self.spatial_transform,
                                epoch=epoch,
                                ...
                                stage='val',
                                write=False)
            loss_sum += loss_recon + loss_cos_sim
            loss_recon_sum += loss_recon
            loss_cos_sim_sum += loss_cos_sim
            if len(self.optimizer) > 1:
                loss = self.loss_fn(net=self.net,
                                    batch=val_batch,
                            r_a = self.R_A, 
        ...
        average_recon_loss = loss_recon_sum / step
        average_cos_sim_loss = loss_cos_sim_sum / step
        if self.is_main_process:
        ...
        self.restore_ema()
        return average_loss, average_recon_loss, average_cos_sim_loss
    ...
    def train(self):
     ...
     #   wandb.login(key="5d9a1c92ef21d246169785ef01b2df570df00cfd")
        self.logger(self.__class__.__name__)
        # wandb.init(
        #     project="BBDM",
        #     config=self.config
        # )
        def compute_alpha(step, total_decay_steps, alpha_init=0.1):
            decay_ratio = min(step / total_decay_steps, 1.0)
            return alpha_init * (1.0 - decay_ratio)
        ...
        try:
            ...
            for epoch in range(start_epoch, self.config.training.n_epochs):
                ...
                for train_batch in pbar:
                    self.global_step += 1
                    self.net.train()
                    # alpha_cos_sim = compute_alpha(self.global_step, total_decay_steps=15000, alpha_init=alpha_cos_sim)
                    losses = []
                    for i in range(len(self.optimizer)):
                        # pdb.set_trace()
                        loss_recon, loss_cos_sim = self.loss_fn(net=self.net,
                            ...
                            spatial_transform=self.spatial_transform,
                                            ...
                                            opt_idx=i,
                                            stage='train')
                        loss = loss_recon + loss_cos_sim * alpha_cos_sim
                        
                        loss.backward()
              #          wandb.log({'train_loss_recon': loss_recon, 'train_loss_cos_sim': loss_cos_sim, 'alpha_cos_sim': alpha_cos_sim}, step=self.global_step)
                    ...
                    #    wandb.log({'train_loss': losses[0]}, step=self.global_step)
                ...
                # validation
                if (epoch + 1) % self.config.training.validation_interval == 1:
                    # if self.is_main_process == 0:
                    with torch.no_grad():
                        self.logger("validating epoch...")
                        average_loss, average_recon_loss, average_cos_sim_loss = self.validation_epoch(val_loader, epoch)
                    #    wandb.log({'val_loss_epoch': average_loss.item(), 'val_loss_recon': average_recon_loss.item(), 'val_loss_cos_sim': average_cos_sim_loss.item()})
                        torch.cuda.empty_cache()
                            ...
                            if self.config.args.save_top:
                                ...
                                if top_key not in self.topk_checkpoints:
                                    ...
                                    if average_recon_loss < self.topk_checkpoints[top_key]["loss"]:
                                        ...
                                        print(
                                            f"saving top checkpoint: average_recon_loss={average_recon_loss} epoch={epoch + 1}")
