...
class BBDMRunner(DiffusionBaseRunner):
    ...
    def loss_fn(self, net, r_a, spatial_transform, batch, epoch, step, opt_idx=0, stage='train', write=True):
        ...
        x = x.to(self.config.training.device[0])
        x_cond = x_cond.to(self.config.training.device[0])
        loss, x0_recon, loss_cos_sim = net(x, x_name, x_cond)
        x0 = x
        if self.reg is not None: 
            x0_recon_reg = F.interpolate(x0_recon, size=(256, 256), mode='bilinear', align_corners=False)
            x0_reg = F.interpolate(x0, size=(256, 256), mode='bilinear', align_corners=False)
            Trans = r_a(x0_recon_reg, x0_recon_reg)
            SysRegist_A2B = spatial_transform(x0_recon_reg,Trans)
            regloss = (x0_reg - SysRegist_A2B).abs().mean()
        ...
        return total_loss, loss_cos_sim
    ...
    def sample(self, net, batch, sample_path, stage='train'):
        ...
        #
        # sample = samples[-1]
        sample = net.sample(x_cond, x_name, clip_denoised=self.config.testing.clip_denoised).to('cpu')
